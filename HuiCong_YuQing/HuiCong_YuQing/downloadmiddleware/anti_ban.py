...
class AntiBanMiddleware(UserAgentMiddleware):
    ...
    def process_exception(self, request, exception, spider):
        self.logger.warning('process_exception ip=%s proxy=%s qsize=%d errmsg=%s',
                            request.meta['bindaddress'][0] if 'bindaddress' in request.meta else '127.0.0.1',
                            request.meta.get('proxy_src', ''),
                            self.ip_queue.qsize() if self.ip_queue else 0, str(exception))
