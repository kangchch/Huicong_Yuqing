...
CHANNEL_SPECIAL_DICT = {
        ...
        u'baidu_droprelated' : u'百度下拉联想词',
        u'so_droprelated' : u'360下拉联想词',
        u'sogou_droprelated' : u'搜狗下拉联想词',
        }
...
if  __name__ == '__main__':
    ...
    #baidu_related_dropdown
    sheet1 = book.add_worksheet(CHANNEL_SPECIAL_DICT['baidu_droprelated'])
    sheet1.write(0, 0, u'频道')
    sheet1.write(0, 1, u'搜索关键字')
    sheet1.write(0, 2, u'负面下拉词')
    sheet1.write(0, 3, u'负面联想词')
    count = 1
    cr = oracleConn.cursor()
    cr.execute("select source, keyword, dropdown, related from huicong_yuqing_info_tbl where source = '{0:s}' or source = '{1:s}' and insert_time > sysdate-6".format('baidu_dropdown', 'baidu_related'))
    for ele in cr:
        sheet1.write(count, 0, u'百度PC')
        sheet1.write(count, 1, ele[1].decode('GBK')  if ele[1] else u'')
        sheet1.write(count, 2, ele[2].decode('GBK')  if ele[2] else u'')
        sheet1.write(count, 3, ele[3].decode('GBK')  if ele[3] else u'')
        count = count + 1
    ...
    #360pc_related_dropdown
    sheet1 = book.add_worksheet(CHANNEL_SPECIAL_DICT['so_droprelated'])
    sheet1.write(0, 0, u'频道')
    sheet1.write(0, 1, u'搜索关键字')
    sheet1.write(0, 2, u'负面下拉词')
    sheet1.write(0, 3, u'负面联想词')
    count = 1
    cr = oracleConn.cursor()
    cr.execute("select source, keyword, dropdown, related from huicong_yuqing_info_tbl where source = '{0:s}' or source = '{1:s}' and insert_time > sysdate-6".format('so_dropdown', 'so_related'))
    for ele in cr:
        sheet1.write(count, 0, u'360PC')
        sheet1.write(count, 1, ele[1].decode('GBK')  if ele[1] else u'')
        sheet1.write(count, 2, ele[2].decode('GBK')  if ele[2] else u'')
        sheet1.write(count, 3, ele[3].decode('GBK')  if ele[3] else u'')
        count = count + 1
    ...
    #sogoupc_related_dropdown
    sheet1 = book.add_worksheet(CHANNEL_SPECIAL_DICT['sogou_droprelated'])
    sheet1.write(0, 0, u'频道')
    sheet1.write(0, 1, u'搜索关键字')
    sheet1.write(0, 2, u'负面下拉词')
    sheet1.write(0, 3, u'负面联想词')
    count = 1
    cr = oracleConn.cursor()
    cr.execute("select source, keyword, dropdown, related from huicong_yuqing_info_tbl where source = '{0:s}' or source = '{1:s}' and insert_time > sysdate-6".format('sogou_dropdown', 'sogou_related'))
    for ele in cr:
        sheet1.write(count, 0, u'搜狗PC')
        sheet1.write(count, 1, ele[1].decode('GBK')  if ele[1] else u'')
        sheet1.write(count, 2, ele[2].decode('GBK')  if ele[2] else u'')
        sheet1.write(count, 3, ele[3].decode('GBK')  if ele[3] else u'')
        count = count + 1
    ...
    cr.execute(
        ...
        UPDATE HUICONG_QUQING_KEYWORD_TBL SET
            ...
            BAIDU_DROPDOWN_FETCH_STATUS=0,
            BAIDU_RELATED_FETCH_STATUS=0,
            SO_DROPDOWN_FETCH_STATUS=0,
            SO_RELATED_FETCH_STATUS=0,
            SOGOU_DROPDOWN_FETCH_STATUS=0,
            SOGOU_RELATED_FETCH_STATUS=0
        '''
    )
    oracleConn.commit()
    cr.close()
